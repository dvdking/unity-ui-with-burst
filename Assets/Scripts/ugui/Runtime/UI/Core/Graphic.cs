...
namespace UnityEngine.UI
{
  ...
  public abstract class Graphic : UIBehaviour, ICanvasElement
  {
    ...
    public virtual float4 colorFloat
    {
      get => new float4(m_Color.r, m_Color.g, m_Color.b, m_Color.a);
      set
      {
        // if (SetPropertyUtility.SetColor(ref m_Color, value)) SetVerticesDirty();
        var newColor = new Color(value.x, value.y, value.z, value.w);
        if (SetPropertyUtility.SetColor(ref m_Color, newColor)) SetVerticesDirty();
      }
    }
        ...
        public virtual void OnRebuildRequested()
        {
            ...
            foreach (var mb in mbs)
            {
                if (mb == null)
                    continue;
                var methodInfo =
 mb.GetType().GetMethod("OnValidate", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
                if (methodInfo != null)
